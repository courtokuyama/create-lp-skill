...
- SVG内にテキストを重ねない（数字は大見出しに、ラベルはHTML側に。チャート上のテキストは視認性が悪い）
- ただしタイムラインのステップ名（BPO→AI構築→納品）や棒グラフの軸ラベルなど、チャートの下に配置するテキストはOK
- カード内の各要素（数字・ラベル・チャート・説明）の高さを固定して、4カード横並びで水平に揃える
- `.stat-card{display:flex;flex-direction:column}` + `.stat-top{height:48px}` で数字行の高さを統一
